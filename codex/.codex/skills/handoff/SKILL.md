...
```bash
${HOME}/.codex/skills/handoff/scripts/state.sh "${PWD}"
```
...
Run `/decisions`, then `/notes` (read and follow each installed SKILL.md). They persist choices and learned facts under `claude_session/`; the handoff then references them by path. Done when both report what they wrote, or that nothing was new.
...
```bash
${HOME}/.codex/skills/handoff/scripts/lint.sh "$HANDOFF"
```
