...
For a Claude-origin handoff, use its original `claude -p --fork-session --resume` command if installed; do not pass a Claude session ID to Codex. Resuming a Codex session appends to it, so prefer reading the recorded transcript for evidence.
...
Run this command explicitly with the shell tool before continuing:

```bash
${HOME}/.codex/skills/pickup/scripts/find.sh "${PWD}"
```

This session is `${CODEX_SESSION_ID}`.
...
```bash
sed -i "/^- HEAD:/a - Picked up: $(date '+%Y-%m-%d %H:%M') by session ${CODEX_SESSION_ID}" "<handoff-path>"
```
...
```bash
codex exec resume <codex-session-id> "<one specific question>"
```
