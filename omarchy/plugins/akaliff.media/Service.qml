...
Item {
  ...
  // True only when the user named a source (source row click / source cycle).
  // runAction also pins preferredPlayerKey as a side effect of ordinary
  // playback, and that kind of pin must stay overridable by auto-follow.
  ...
  function selectActivePlayer() {
    ...
    if (preferred && (preferred.isPlaying || root.preferredIsExplicit)) return preferred
  ...
  // Clamped absolute seek rather than Mpris Seek(offset): per the MPRIS spec a
  // relative seek past the end of a track acts like Next, so a forward skip
  // near the end of a podcast episode would jump to the next one.
  function seekTo(seconds) {
    var player = activePlayer
    if (!player || !player.canSeek || !player.positionSupported) return false
    var end = player.lengthSupported && player.length > 0 ? player.length : seconds
    player.position = Math.max(0, Math.min(end, seconds))
    return true
  }
  ...
  function seekBy(deltaSeconds) {
    ...
    return seekTo(player.position + deltaSeconds)
  }
  ...
  function playerForAction(action, targetKey) {
    ...
    if ((action === "pause" || action === "playPause") && !root.preferredIsExplicit) {
  ...
  function runAction(action, showFeedback, targetKey) {
    ...
    if (handled && key && !root.preferredIsExplicit) preferredPlayerKey = key
