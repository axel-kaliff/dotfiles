...
Panel {
  ...
  // Which account the numbers belong to. The usage record never carries it —
  // that is identity, not usage — so read it straight off the Claude CLI's
  // config, the same file the CLI itself trusts to say who is logged in.
  property string claudeAccount: ""
  readonly property string claudeConfigPath:
    (Quickshell.env("CLAUDE_CONFIG_DIR") || Quickshell.env("HOME") || "") + "/.claude.json"
  ...
  // Only Claude keeps its account on disk here; the other providers get no
  // badge rather than a borrowed one.
  function accountLabel(p) {
    return p && String(p.providerId) === "claude" ? root.claudeAccount : ""
  }
  ...
  // jq keeps the 100KB config off the UI thread: only the one field crosses.
  // A logged-out CLI prints nothing, which blanks the badge rather than
  // leaving yesterday's account on screen.
  Process {
    id: accountProbe
    running: false
    command: ["jq", "-r", ".oauthAccount.emailAddress // empty", root.claudeConfigPath]

    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.claudeAccount = String(text || "").trim()
    }
  }
      ...
      Flickable {
        ...
        Column {
          ...
          PanelHero {
            ...
            detail: root.accountLabel(root.provider)
