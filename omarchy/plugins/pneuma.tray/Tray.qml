...
BarWidget {
  ...
  // Clone note: the only behavioural change from omarchy.tray. With
  // `alwaysExpanded` the drawer is simply always open, so the icons are just
  // there instead of appearing when the pointer finds them -- hovering to
  // discover what is in the tray is the interaction easy mode exists to remove.
  // The chevron goes with it: it only ever meant "there is more behind here",
  // which is not true once the drawer is always open. Its right-click route to
  // managing pinned/hidden items goes too -- that is a tuning job for the
  // normal bar, which still runs the stock omarchy.tray.
  readonly property bool alwaysExpanded: setting("alwaysExpanded", false) === true
  property real revealProgress: (expanded || alwaysExpanded) ? 1 : 0
  ...
  Component {
    ...
    Item {
      ...
      readonly property int drawerBlockWidth: root.allItems.length > 0 ? (root.alwaysExpanded ? 0 : expandIcon.implicitWidth) + root.drawerExtent : 0
  ...
  Component {
    ...
    Item {
      ...
      readonly property int drawerBlockHeight: root.allItems.length > 0 ? (root.alwaysExpanded ? 0 : expandIcon.implicitHeight) + root.drawerExtent : 0
      ...
      Item {
        ...
        Item {
          id: trayClip
          y: root.alwaysExpanded ? 0 : expandIcon.height
