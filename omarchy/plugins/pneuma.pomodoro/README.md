...
| Panel | `S` | Skip phase |
| Panel | `,` | Show/hide the session lengths |
| Panel | `Esc` | Close |
...
`,`) folds out a row per phase with `−`/`+` steppers. Steps land on multiples
of 5 between 1 and 180 minutes. Changing the length of the phase currently on
the clock keeps the time already spent and moves the deadline, so stretching
focus mid-session buys the extra minutes right away.

Everything, lengths included, also lives inline on the bar layout entry in
`~/.config/omarchy/shell.json` — the panel writes to the same place, so the two
routes cannot disagree. Numbers need `--json` or they land as strings:
