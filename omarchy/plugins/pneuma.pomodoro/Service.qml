...
Item {
  ...
  // Re-length a phase that is already on the clock. Elapsed time is kept and
  // the deadline moves, so stretching focus mid-session buys the minutes just
  // asked for rather than applying only to the next one. Shortening below what
  // is already spent leaves no time on the clock and the next tick rolls the
  // phase over, which is the honest reading of "make this a 5 minute focus"
  // seven minutes in.
  function retargetPhase(target, minutes) {
    if (phase !== target || totalMs <= 0) return
    nowMs = Date.now()
    var delta = minutes * 60000 - totalMs
    totalMs += delta
    if (running) endsAt = Math.max(nowMs, endsAt + delta)
    else pausedMs = Math.max(0, pausedMs + delta)
    scheduleSave()
  }
