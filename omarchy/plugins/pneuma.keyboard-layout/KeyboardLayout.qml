...
BarWidget {
  id: root
  moduleName: "pneuma.keyboard-layout"
  ...
  readonly property string layoutCode: KeyboardLayoutModel.shortLabel(layoutFull, layoutBriefs)
  // Clone note: the only change from omarchy.keyboard-layout. `flags` maps the
  // short code the model derives ("EN", "SV") to whatever should be shown for
  // it -- a flag emoji here. Anything unmapped keeps the code, so a layout that
  // has no entry still labels itself rather than going blank.
  readonly property var flagLabels: setting("flags", ({}))
  readonly property string layoutLabel: {
    var mapped = root.flagLabels[root.layoutCode]
    return (typeof mapped === "string" && mapped !== "") ? mapped : root.layoutCode
  }
