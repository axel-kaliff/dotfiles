...
Item {
  ...
  component ModuleSlot: Item {
    ...
    // An open panel highlights its module the way the macOS menu bar
    // highlights an open menu title: a translucent capsule under the label
    // instead of an accent mark on the bar edge. Fill and hairline are the
    // [controls] selected-state tokens, so the pill matches every other
    // selected control that sits on glass.
    BorderSurface {
      readonly property int inset: Style.space(3)

      visible: opacity > 0
      opacity: slot.panelOpen && !slot.dragSource ? 1 : 0
      anchors.fill: parent
      anchors.topMargin: root.vertical ? 0 : inset
      anchors.bottomMargin: root.vertical ? 0 : inset
      anchors.leftMargin: root.vertical ? inset : 0
      anchors.rightMargin: root.vertical ? inset : 0
