//! Active keyboard group over cosmic-comp's keyboard-layout protocol.
//! Adapted from cosmic-applet-input-sources (System76, GPL-3.0-only).
    ...
    fn new_capability(
        ...
        if capability == Capability::Keyboard && self.keyboard.is_none() {
            ...
            if let Some(keyboard_layout) = keyboard_layout {
                let _ = block_on(self.sender.send(Event::KeyboardLayout(keyboard_layout.clone())));
                self.keyboard = Some(Keyboard { seat, keyboard_layout });
            }
        }
    }
...
cctk::wayland_client::delegate_noop!(AppData: ignore wl_keyboard::WlKeyboard);
